...
5、mysql本地登录验证流程
登录方式分为基于蓝鲸平台认证登录和基于本地mysql数据库用户验证登录

5.1、本地开发时，在`local_settings.py`中修改开发环境为DEBUG=True
![local_settings.py](./docs/img/Debug.png)

5.2、在`blueapps/account/components/bk_token`目录中的`middlewares.py`修改中间件LoginRequiredMiddleware
验证当前环境是否为开发环境，从而验证用户登录方式为蓝鲸平台登录或者本地登录
![middlewares.py](./docs/img/middlewares.png)

5.3、在`blueapps/account/components/bk_token`目录中的`backends.py`设置用户登录的验证方式LoginBackend或者TokenBackend
![backends.py](./docs/img/backends.png)

5.4、在`blueapps/account/sites/open`目录中的`conf.py`中将LoginBackend(本地验证)或者TokenBackend(蓝鲸登录)注册进USER_BACKEND中
![backends.py](./docs/img/conf.png)
